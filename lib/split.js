 ...
 * @returns
 */
function split_esbuild_output_chunk(code, working_dir, external_packages = []) {
    //begin split
            ...
            for (let x = i + 1; x < content.length; x++) {
                ...
                if (char_perfix === '\n') {
                    if (char_perfix_txt.endsWith('.ts\n') ||
                        ...
                        if (stream.trim() !== '') {
                            const export_res = (0, export_calc_1.export_calc)(stream);
                            export_hashmap.set(file_index, {
                                index: file_index,
                                file_location: (0, disk_path_1.resolve_disk_path)(file_location, working_dir),
